...
def draw_settings_panel(state):
    ...
    if imgui.button("Camera 1 front"):
        print("Camera front")
        state.requested_camera_1 = 0

    imgui.same_line()

    if imgui.button("Camera 1 back"):
        print("Camera back")
        state.requested_camera_1 = 1

    imgui.same_line()

    if imgui.button("Camera 1 manipulator"):
    ...
    if imgui.button("Camera 2 front"):
        print("Camera front")
        state.requested_camera_2 = 0

    imgui.same_line()

    if imgui.button("Camera 2 back"):
        print("Camera back")
        state.requested_camera_2 = 1

    imgui.same_line()

    if imgui.button("Camera 2 manipulator"):
        print("Camera manipulator")
